...
-- This function is updated to be compatible with a legacy `auth.users` schema.
-- It now provides a default value for the `email_change` column, which was causing
-- login to fail with a "Scan error" because the column was being set to NULL.
...
BEGIN
  IF NEW.phone IS NOT NULL AND NEW.phone <> '' THEN
    ...
    VALUES (
      ...
      now(),
      NEW.phone,
      '', '', '', '', '', '' -- Added one more empty string for email_change
